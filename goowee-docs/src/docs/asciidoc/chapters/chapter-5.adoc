...
Now we are going to finish our first Goowee application.
...
. The class name starts with `T`. This is a Goowee convention to let us immediately realize when we are working on a domain object. `T` stands for _Table (I know, it’s such an original decision)._
...
We don’t like writing the _Business Logic_ in controllers. It is an _anti-pattern_. Goowee uses controllers as a way to implement the _User Interface_.
...
import goowee.elements.contents.*
...
Now, with a bit of luck, we should be able to run our first complete Goowee application:
...
This chapter closes the first round on the Goowee basics.
